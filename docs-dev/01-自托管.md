...
| `worker` | 同后端镜像 | **Celery Worker**：账单解析等异步任务 |
...
> 当前目录为仓库根目录（含 `docker-compose.yaml`）。预置镜像不含权重，需本机拉取到固定路径。
...
平台内置 MCP 服务，把当前用户的账本以**只读**方式提供给 Claude Code、Cursor 等支持 MCP 的 AI 客户端。本节只讲部署与配置，端点、鉴权与能力清单见 [MCP 服务](https://trans.dhr2333.cn/docs/developer/mcp)。

### 服务与路由

- Compose 中的 **`beancount-trans-mcp`** 与后端共用同一镜像，以第二个进程监听容器内 **8001**，端点路径为 **`/mcp`**（Streamable HTTP，单 JSON 响应、无会话）。
- 前端 Nginx 已把 **`/mcp`** 及 OAuth 相关路径（`/.well-known/*`、`/authorize`、`/token` 等）反代到 MCP 服务与后端，因此对外仍只需暴露 **38001** 一个端口。

### 环境变量

`.env` 中至少配置以下项（完整说明见 `.env.example` 的「MCP 服务（AI 客户端接入）」一节）：

```shell
MCP_RESOURCE_SERVER_URL=http://your-domain.com:38001/mcp
MCP_ISSUER_URL=http://your-domain.com:38001
MCP_ALLOWED_HOSTS=your-domain.com:38001
MCP_ALLOWED_ORIGINS=http://your-domain.com:38001
```

未配置 `MCP_RESOURCE_SERVER_URL` 时服务不校验访问令牌（仅适合本地调试），**生产必须配置**。`MCP_ALLOWED_HOSTS` / `MCP_ALLOWED_ORIGINS` 需包含客户端实际访问的域名与端口，否则请求会因 Host 校验失败返回 400。客户端使用自定义回调 scheme（如 Cursor）时，还需在 `MCP_OAUTH_REDIRECT_URI_SCHEMES` 中追加该 scheme。

### 获取访问凭证

两种方式任选其一：

1. **个人访问令牌**：登录 Web 应用 → **个人设置 → 访问令牌 → 新建令牌**。明文令牌**仅显示一次**，请立即复制保存；泄露或不再使用时在同一页面点「撤销」即可立即失效。
2. **OAuth 2.1**：客户端按标准流程自动完成动态注册与浏览器授权，无需手工复制令牌。
...
claude mcp add --transport http beancount-trans \
  http://your-domain.com:38001/mcp \
...
```json
{
  "mcpServers": {
    "beancount-trans": {
      "type": "http",
      "url": "http://your-domain.com:38001/mcp",
      "headers": { "Authorization": "Bearer bct_xxxxxxxxxxxxxxxx" }
    }
  }
}
```

面向使用者的操作步骤与常见问题见 [接入 AI 客户端](https://trans.dhr2333.cn/docs/%E6%93%8D%E4%BD%9C%E6%8C%87%E5%8D%97/connect-ai-client)。
...
3. `docker-compose.yaml`：删除或注释 **`beancount-trans-postgres`**；在 **`beancount-trans-backend`**、`worker`、`beancount-trans-beat` 上移除对 **`beancount-trans-postgres`** 的 **`depends_on`**。  
...
flowchart TD
  ...
  dlModel[下载bert-base-chinese至pretrained_models]
  envFile[复制并编辑.env]
