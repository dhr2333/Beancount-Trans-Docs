...
标记为 `—` 的变量未出现在模板中，仅在 `settings.py` 提供默认值。完整说明见 [MCP 服务](https://trans.dhr2333.cn/docs/developer/mcp)。
...
MCP 与 OAuth 相关路径挂在**域名根**（不带 `/api` 前缀），经前端 Nginx 转发的映射关系与鉴权细节见 [MCP 服务](https://trans.dhr2333.cn/docs/developer/mcp)。
