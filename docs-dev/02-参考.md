...
| 变量名 | 必填级别 | 默认值（示例） | 说明 | 模板 |
| :--- | :--- | :--- | :--- | :--- |
| `ASSISTANT_DEEPSEEK_API_KEY` | 可选 | 无 | 平台级 DeepSeek API Key；用户未在「输出配置」填写时作为回退 | both |
| `ASSISTANT_MODEL` | 可选 | `deepseek-chat` | 助手默认 chat 模型；用户可在前端「深度思考」开关临时切换为 `deepseek-reasoner` | both |
...
用户侧也可在平台 **输出配置** 填写个人 DeepSeek Key，优先于平台 Key。使用说明见 [AI 账本助手](/docs/操作指南/ai-ledger-assistant)。
