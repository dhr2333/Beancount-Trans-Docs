...
- 已了解该来源面向用户的导出方式，并需在《账单导出方法》中补充说明（见下文第 8 步）。
- 本地能跑起后端，并能执行 `pytest`。
...
- **用户文档**：更新《账单导出方法》（`docs/02-操作指南/账单导出方法.md`），在「支持以下账单」下新增该来源的小节，说明用户如何导出该账单；线上页面见 [bills-export-methods](https://trans.dhr2333.cn/docs/%E6%93%8D%E4%BD%9C%E6%8C%87%E5%8D%97/bills-export-methods)，同时把 frontmatter 的 `modify_time` 更新为本次改动时间。
