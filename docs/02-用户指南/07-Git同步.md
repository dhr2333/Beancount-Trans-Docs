...
modify_time: 2025-12-10 15:05:10
authors: dhr2333
doc_status:
published:
...
```mermaid
flowchart TD
    subgraph A[第一阶段：启用Git同步]
        direction TB
        A1[用户在平台启用Git同步] --> A2{选择创建方式}
        A2 -->|基于模板| A3[创建标准结构仓库]
        A2 -->|空仓库| A4[创建空仓库]
        A3 --> A5[下载Deploy Key]
        A4 --> A5
...
```shell
# 方式一：使用配置的 SSH 别名（推荐）
git clone gitea-beancount:beancount-trans/{用户ID}-beancount.git ~/my-beancount

# 方式二：使用完整的 SSH URL
git clone ssh://git@gitea.dhr2333.cn:30022/beancount-trans/{用户ID}-beancount.git ~/my-beancount
```
...
- [VS Code Beancount 插件](https://marketplace.visualstudio.com/items?itemName=Lencerf.beancount)
